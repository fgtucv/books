const a=async()=>{try{return await fetch("https://books-backend.p.goit.global/books/category-list").then(a=>a.json())}catch(a){return a}},t=document.querySelector(".category__list");a().then(a=>{let e;e="",e=a.map(a=>`
        <li class="category__item">
            <span class="category__span">${a.list_name}</span>
        </li>`).join(""),t.insertAdjacentHTML("beforeend",e)});const e=async a=>{try{return await fetch(`${a}`).then(a=>a.json())}catch(a){return a}},s=document.querySelector(".magazine");function o(a,t=!1){let e="";!1===t?e=a.map(a=>`
